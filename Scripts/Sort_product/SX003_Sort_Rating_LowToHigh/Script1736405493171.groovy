...
// Check the sort menu and select "Rating: Low to High"
...
products.each { product ->
    def ratingText = product.getText().trim() // Get the rating as a string
    
    if (ratingText.isEmpty()) {
        ratings.add(0.0) // If the string is empty, add a value of 0.0 to the list
    } else {
        ratings.add(Double.parseDouble(ratingText)) // Convert to a double for comparison
    }
}
...
WebUI.delay(5)
// Close the browser after the verification
WebUI.closeBrowser()
